...
#define UTIL_ASSERT_DEBUG(cond, e, logName, logLevel)                       \
    if (cond) ;                                                             \
    else rcfutil::VarArgAssert(__FILE__, __LINE__, #cond)
#endif
...
#define UTIL_ASSERT_DEBUG_GCC_33_HACK (const rcfutil::VariableArgMacro<rcfutil::ThrowFunctor> &)
#else
#define UTIL_ASSERT_DEBUG_GCC_33_HACK
#endif

    DECLARE_VARIABLE_ARG_MACRO( UTIL_ASSERT_DEBUG, rcfutil::AssertFunctor );
    #define UTIL_ASSERT_DEBUG(expr, e, logName, logLevel)                                    \
        if (expr);    \
        else\
            UTIL_ASSERT_DEBUG_GCC_33_HACK                                              \
            rcfutil::VariableArgMacro<rcfutil::AssertFunctor>(#expr)    \
                .init(                                                          \
                    "",                                                         \
                    "",                                                         \
                    __FILE__,                                                   \
                ...
                .cast( (rcfutil::VariableArgMacro<rcfutil::AssertFunctor> *) NULL)     \
                .UTIL_ASSERT_DEBUG_A
