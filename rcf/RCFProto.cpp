...
void RcfProtoServer::bindService(Service & service, std::string serviceName)
{
    mProtobufServices[serviceName] = &service;
}

void RcfProtoServer::bindService(Service * service, std::string serviceName)
{
    //mProtobufServices.insert(std::pair<std::string,google::protobuf::Service*>(serviceName, service));
	mProtobufServices[serviceName] = service;
}
    ...
    int                     methodId)
{
    ...
    std::map<std::string,google::protobuf::Service *>::iterator iter = mProtobufServices.find(serviceName);
